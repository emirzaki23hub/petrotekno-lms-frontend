...
const PdfViewer: React.FC<PdfViewerProps> = ({ url, title }) => {
  ...
  // --- Remote Pointer & Keyboard Listener ---
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      // Cegah scroll default saat menggunakan arrow keys jika fokus di viewer
      if (["ArrowRight", "ArrowLeft", "PageUp", "PageDown"].includes(event.key)) {
  ...
  const renderPage = async (num: number, pdf: pdfjsLib.PDFDocumentProxy) => {
    ...
    // Render pada scale 2.0 (High DPI) agar tajam saat dizoom lewat CSS
  ...
  // --- Variabel Animasi & Zoom ---
  const variants = {
    enter: (direction: number) => ({
      opacity: 0,
      x: direction > 0 ? "20%" : "-20%", // Menggunakan % agar lebih aman di layar kecil
      scale: zoomLevel,
    }),
    center: {
      zIndex: 1,
      ...
      >
        ...
        <div
          className={cn(
            ...
            "overflow-x-hidden overflow-y-auto" // Kunci horizontal agar tidak muncul scrollbar saat slide ganti halaman
          )}
        >
          <AnimatePresence initial={false} mode="popLayout" custom={direction}>
            <motion.img
              ...
              animate="center"
              exit="exit"
              className="max-h-full object-contain shadow-2xl origin-center pointer-events-auto"
